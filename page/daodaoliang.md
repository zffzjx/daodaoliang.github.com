...
#### <i class="fa fa-dot-circle-o"></i> 易餐宝店铺助手(网络版)

暂无-占位

#### <i class="fa fa-dot-circle-o"></i> 易餐宝店铺助手(本地版)
...
#### <i class="fa fa-dot-circle-o"></i> cusflo客流画像产品

* **软件环境:** `CentOS`
* **硬件环境:** `Linux 服务器`
* **开发环境:** `Pycharm + Python + Redis + Mysql + Nginx + C++ + Shell`
* **项目简述:** 一套数据挖掘和数据分析报告生成系统，按照加密的数据传输协议给百度对应的开放接口进行客流数据的流式传输，并定期获取百度生成的客流画像数据进行转存和对外开放接口，已经提供APP端展示交互；
* **责任简述:** 数据挖掘和分析服务的开发，画像报告的生成，正式上线并运行服务与1000+商户；


#### <i class="fa fa-dot-circle-o"></i> wi-beacon客流硬件产品(基于OpenWRT的智能路由器)

* **软件环境:** `OpenWRT(Linux变种)`
* **硬件环境:** `TpLink路由器`
* **开发环境:** `Python + Shell + Sqlite3 + C`
* **项目简述:** 一套基于路由器的客流分析硬件系统, 主要实现改造路由器系统, 通过`802.11`协议的解析, 完成对手机等手持设备的硬件网卡信息进行捕获、上传、分析；
* **责任简述:** 负责数据的解析和分布式上传，产品主页[wi-beacon.com][7]；
